...
class TestPatternMatcher(TestPatternMatcherBase):
    ...
    @parametrize("dtype", [torch.float32, torch.bfloat16])
    @parametrize("dynamic", [True, False])
    def test_static_smooth_quant_u8s8_with_int_mm(
        self, has_bias, dtype, dynamic
    ):
        r"""
        This testcase checks if we can match the static SmoothQuant u8s8 int8 linear
        pattern from Torchao.  Compared to the symmetric s8s8 pattern, the asymmetric
        (u8) activation introduces a zero-point correction term that is subtracted from
        the intermediate result before the weight-scale multiply.

        The pattern is:
            (no bias) reshape -> _int_mm -> convert_element_type -> mul(x_scale)
                      -> sub(zp_correction) -> mul(w_scale) -> reshape
        or
            (with bias) pattern_no_bias -> add(bias)

        The zp_correction is a precomputed constant:
            zp_correction = a_zp * a_scale * sum_of_weight_rows  (shape [out_features])
        """
        if dtype == torch.bfloat16 and not torch.ops.mkldnn._is_mkldnn_bf16_supported():
            return
        M = 16
        ...
        class Mod(torch.nn.Module):
            def __init__(
                self, dtype: torch.dtype, has_bias: bool
            ):
                super().__init__()
                self.dtype = dtype
                self.has_bias = has_bias
                ...
                # Per-tensor (scalar) activation scale – required for the
                # x_scale_is_scalar branch that folds zp into an effective bias.
                self.a_scale = torch.rand([1], dtype=dtype) * 0.01 + 0.01
                self.b_scale = torch.rand([out_feature], dtype=dtype) * 0.01 + 0.01
                self.bias = torch.rand([out_feature], dtype=dtype) if has_bias else None
                # Simulate asymmetric (u8) activation: a_zp is a non-zero scalar.
                a_zp = torch.randint(1, 10, [1]).to(dtype)
                # Precompute the zero-point correction constant:
                #   zp_correction_i = a_zp * a_scale * sum_j(b_ij)  for each output channel i
                w_row_sums = self.b.to(dtype).sum(dim=0)  # [out_feature]
                self.zp_correction = (
                    a_zp * self.a_scale * w_row_sums
                )  # [out_feature]
            ...
            def forward(self, a):
                ...
                c = c.to(self.dtype)
                c = c * self.a_scale
                # u8s8 zero-point correction (constant)
                c = c - self.zp_correction
                c = c * self.b_scale
                if self.has_bias:
                    c = c + self.bias
                c = c.reshape(out_shape)
